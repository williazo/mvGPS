...
#' @param C list of numeric matrices of length \code{m} of dimension \code{n} by \code{k} designating values of the confounders for each exposure value
...
#' @export
#'
mvGPS <- function(D, C, trim_w=FALSE, trim_quantile=0.99){
    m <- ncol(D)
    n <- nrow(D)
